...
export function PaperSelector({
  ...
  useEffect(() => {
    if (allPapers && allPapers.length > 0) {
      // First filter by paper type if specified
      let filteredPapers = allPapers;
      if (paperTypeFilter !== "all") {
        filteredPapers = allPapers.filter(paper => paper.paper_type === paperTypeFilter);
      }
      
      // Then filter by test if testId is specified
      if (testId) {
      ...
      setAvailablePapers(filteredPapers);
  ...
  }, [allPapers, testId, paperTypeFilter]);
              ...
              {filteredGroups.map((group) => (
                ...
                >
                  ...
                  <div className="space-y-3">
                    {paperTypeFilter !== "answer" && group.questionPaper && (
                      <div 
                    ...
                    {paperTypeFilter !== "question" && group.answerKey && (
                      <div 
                        className={`p-3 border rounded-md cursor-pointer transition-colors ${
